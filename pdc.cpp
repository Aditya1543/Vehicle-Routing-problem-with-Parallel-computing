...
self._total_time[from_node][to_node] = int(self.service_time(data,
from_node) + self.travel_time(data, from_node,to_node))
...
def _init_(self, data, routing, assignment):
...
def p(self,vehicle_id,capacity_dimension,time_dimension):
...
plan_output += ' {0} Load({1}) Time({2}, {3}) ->'.format(node_index,
...
results = [executor.submit(self.p,i,capacity_dimension,time_dimension) for
i in range(self.data.num_vehicles)]
